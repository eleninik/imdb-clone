...
<div class="top">
    ...
    <!-- Title and description of series -->
    <section class="series-description">
        <div class="series-title-rating">    
            <h1 class="series-title gold"> {{$series->title}}</h1>  
                <div class="series-avarage">
                <label class="star checked" for="star-10"></label> {{$avarageRating}} <i class="fas fa-star"></i> <p>Votes: {{$votes}}</p>
                </div>
        </div>            

        <ul class="series-time">
            <li class="grey">Released in {{$series->year}}</li>
            <li class="white">|</li>
            ...
            <li class="grey">Season  <a class="gold" href ="/series/{{$series->id}}/seasons/{{ $season->season_number}} "> {{ $season->season_number}}</a></li>
            @endforeach
            <li class="white">|</li>
            <li class="grey">{{$series->age_rating}} </li>
        </ul>
        ...
        {{ csrf_field() }}
        <input type="hidden" name="rateable_id" value="{{$series->id}}" />
        <input type="hidden" name="model" value="series" />
...
<section class="series-mainContent">
    ...
    <section class="seriesInfo">
        ...
        <div class="together">
                ...
                {{ csrf_field() }}
                <input type="hidden" name="item_id" value="{{$series->id}}" />
                <input type="hidden" name="type" value="series" />
                <button type="submit" class="series-addwatchlist"> Add to Watchlist</button>
            </form> 

            <form action="
            mailto:someone@example.com?Subject=Check%20out%20this%20tv-series!
            &body=Hi,%20I%20think%20you'd%20really%20like%20this%20tv-series!
            %0Ahttp://dev.amdb.lndgrn.xyz/series/{{ $series->id }}
            " 
            method="post">
            ...
            @foreach ($directors as $director)  
                ...
                <div class="info-series">
                        <li class="grey">Find all episodes</li>
                        @foreach ($seasons as $season)
                        <li class="movieData"><a class="gold" href ="/series/{{$series->id}}/seasons/{{ $season->season_number}} ">Season {{ $season->season_number}} </a></li>
                        @endforeach 
                 </div>
                <div class="awards">
        ...
        <iframe class="trailer" src="https://youtube.com/embed/{{$series->trailer}}" frameborder="0" gesture="media" allow="encrypted-media" allowfullscreen></iframe>        
            ...
            @if (\Auth::user())
                <div id="leave-review" class="collapse"> 
                    <form class="form-review" action="/review" method="post">
                        ...
                        <input type="hidden" name="model" value="series" />   
                            ...
                            <p class="current-rating"> Your current rating: <label class="star checked" for="star-10"></label> {{$userRating->rating}} </p>
                            @endif
                        <div class="form-group series-rating">
                            <input type="hidden" name="{{ \Auth::user()->name }}">
                            <input type="hidden" name="{{ \Auth::user()->id }}">
                            <input type="hidden" name="reviewable_id" value="{{$series->id}}" />
                            <input type="hidden" name="model" value="series" />  
                ...
                @foreach ($reviews as $review)
                    <p class="review-info"> <img class="profil-mini-mini" src="{{$review->user['profile_picture']}}"> | Review by {{$review['user_name']}} | {{$review['created_at']}} </p> 
                        <div class="review-body">
                            <div class="review-title">
                                <h5> Series: {{$series->title}}</h5>  <h4><i class="far fa-star small"></i> {{$review->rating['rating']}}/10 </h4>
                            </div>
